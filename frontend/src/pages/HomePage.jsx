...
// Put a real image at: frontend/public/hero.webp
const HERO_FALLBACK = "/hero.webp";

function useDeferQuery(ms = 1000) {
  ...
  useEffect(() => {
    ...
    if (typeof w.requestIdleCallback === "function") {
      idleId = w.requestIdleCallback(enable, { timeout: ms });
      t = w.setTimeout(enable, ms);
    } else {
      t = w.setTimeout(enable, ms);
    }
...
function useInView(rootMargin = "300px") {
  ...
  useEffect(() => {
    ...
    const el = document.getElementById("popular-sentinel");
    if (!el) return;
...
function ProgressiveHeroImage({ url, alt }) {
  const [ready, setReady] = useState(false);

  const src = useMemo(() => (url ? getOptimizedUrl(url, 1200) : null), [url]);
  const srcSet = useMemo(
    () => (url ? generateSrcSet(url, [600, 800, 1200]) : null),
    [url]
  );
  ...
  return (
    <img
      ...
      width={1200}
      height={800}
      loading="lazy" // don't compete with LCP
      decoding="async"
...
export default function HomePage() {
  // Defer above-fold data a bit, below-fold more
  const enableAboveFold = useDeferQuery(700);
  const enableBelowFold = useDeferQuery(1200);

  const popularInView = useInView("350px");
  const enablePopular = enableBelowFold && popularInView;

  const { data: featuredPosts = [], isLoading: featuredLoading } =
    useGetFeaturedPostsQuery(1, {
      skip: !enableAboveFold,
    });
  ...
  const {
    ...
    isError: recentError,
  } = useGetRecentPostsQuery(6, { skip: !enableBelowFold });

  const { data: categoriesData, isLoading: categoriesLoading } =
    useGetCategoriesQuery(undefined, {
      skip: !enableBelowFold,
    });
  const categories = categoriesData?.categories || [];

  const { data: popularPosts = [] } = useGetPopularPostsQuery(3, {
    skip: !enablePopular,
  });
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-8">
              ...
              <p className="text-xl text-gray-400 max-w-2xl leading-relaxed">
                Dive deep into cutting-edge technologies, expert insights, and
                industry best practices. Your journey to mastery starts here.
              </p>
            ...
            {/* Featured Card (LCP image static first, then fades in dynamic) */}
            <div ref={heroCardRef}>
              <Link
                to={
                  featuredPost
              ...
              >
                <div className="absolute inset-0 bg-gradient-to-r from-emerald-500 to-cyan-500 rounded-3xl blur-xl opacity-20 group-hover:opacity-30 transition-opacity" />
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    <div className="flex flex-wrap gap-2 mb-3">
                      ...
                      {featuredPost?.categories?.map((cat, i) => (
                        <span
                          key={i}
                          className="px-3 py-1 bg-white/10 backdrop-blur-md text-white text-[10px] font-bold rounded-full border border-white/20"
                        >
                          {cat.title || cat}
                        </span>
                      ))}
                    ...
                    <h3 className="text-2xl font-bold text-white mb-2 line-clamp-2 group-hover:text-emerald-400 transition-colors">
                      {featuredLoading
                        ...
                        : featuredPost?.title || "Explore the latest articles"}
                    </h3>
                    ...
                    {featuredPost?.excerpt ? (
                      ...
                      <p className="text-gray-400 text-sm mb-4 line-clamp-2">
                        {featuredLoading
                          ? "Fetching content…"
                          : "Open the blog to discover new content."}
                      </p>
                    )}
          ...
          {categoriesLoading ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              {[...Array(5)].map((_, i) => (
                <div
                  key={i}
                  className="h-40 bg-slate-800/50 rounded-2xl animate-pulse"
                />
              ))}
            </div>
          ) : (
                  ...
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4 flex flex-wrap gap-2 pr-4">
                      {post.categories?.map((cat, i) => (
                        <span
                          key={i}
                          className="px-3 py-1 bg-emerald-500/20 backdrop-blur-sm text-emerald-400 text-[10px] font-bold rounded-full border border-emerald-500/30 whitespace-nowrap"
                        >
                          {cat.title || cat}
                        </span>
                      ))}
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-white/10">
                      ...
                      <span className="text-gray-500 text-xs">
                        {post.readingTime || post.estimatedReadingTime || 5} min
                      </span>
                    </div>
                  </div>
                </Link>
        ...
        <section ref={popularRef} className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 grid md:grid-cols-3 gap-6">
            ...
            <div className="bg-purple-500/10 backdrop-blur-md rounded-2xl p-6 border border-purple-500/20 transform-gpu">
              ...
              <p className="text-gray-400 text-sm">
                Engage with thousands of fellow developers
              </p>
            </div>
          </div>

          {/* Sentinel: popular query starts only when near viewport */}
          <div id="popular-sentinel" className="hidden lg:block" />
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-emerald-500 to-cyan-500 rounded-3xl p-12">
            ...
            <div className="relative max-w-3xl mx-auto text-center">
              ...
              <p className="text-emerald-100 text-lg mb-8">
                Get weekly insights, tutorials, and exclusive content delivered
                to your inbox.
              </p>
